...
* H.01, H.02 — формат заголовков 1-2 уровней,
* H.07 — отступы между заголовками.

ВАЖНО О ГРАНИЦАХ ЭТОГО ТЕСТА (читать внимательно):

Текущий валидатор смотрит форматирование на уровне run-ов
(прямых w:rPr-атрибутов), а python-docx для заголовков run-ы не
форматирует явно — наследование идёт от стиля Heading{N}. Поэтому
если экспортёр перестанет править стили (т. е. _apply_heading_styles
отключится и Heading1 вернётся к дефолтному «синий Cambria»),
H.01/H.02 этого НЕ заметят — нарушений не будет, но документ
визуально неправильный.

Первичный guard на регрессии вёрстки — tests/test_exporter_styles.py:
он напрямую читает styles.xml и проверяет, что Heading1, Heading1Char
не содержат «365F91», «majorHAnsi» и содержат «Times New Roman».
Этот тест (roundtrip) — вторичный: он показывает, что нормоконтроль
самой системы доволен сгенерированным документом, и что после правок
не сломались F.01-F.03/T.01-T.05 (вот эти проверки чувствительны
к run-уровню и реально стрельнут на регрессии в _apply_normal_style).
...
LAYOUT_CHECK_CODES = frozenset(
    {
        ...
        "H.01",  # формат заголовка 1 (жирный, центр, TNR, ВЕРХ. регистр)
        "H.02",  # формат заголовка 2 (жирный, слева)
