...
## Фаза 2 — Расширение покрытия и продакшен — в активной разработке

**Покрытие каталога: 94 проверки (94%).** 13 из 15 категорий закрыты
полностью (F, T, S, H, B, M, L, C, A, P, K, V, X). Не реализованы только:

- [ ] **I.08** (DPI ≥ 150 для изображений) — требует загрузки media-file
      и анализа метаданных Pillow.
- [ ] **I.09** (рисунок выровнен по центру) — требует поля alignment у
      Figure в модели.
- [ ] **R.02** (порядок литературы алфавит / по упоминанию).
- [ ] **R.03** (обязательные поля для типа источника) — требует парсера
      полей BibliographyEntry (сейчас только raw).
...
Все R.02-R.13 — это расширенная аналитика библиографии, требует парсинга
полей `BibliographyEntry.fields` (автор, заглавие, год, издательство,
страницы) из raw-текста по ГОСТ Р 7.0.100-2018.
