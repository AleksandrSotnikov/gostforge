...
библиографическая цитата). Сейчас параграф в Streamlit-конструкторе —
это один `text_area` без локального форматирования, без inline-формул
и без перекрёстных ссылок. Модель документа уже поддерживает
`InlineElement = TextRun | CrossRef` — Фаза 2.5 поднимает эту структуру
в UI.

Ключевые пункты (без детализации — см. спецификацию):

- [ ] **Модель:** новые типы `InlineFormula`, `Citation`,
      `CrossRef.prefix`, `TextRun.underline`; bump `SCHEMA_VERSION → 0.3.0`.
- [ ] **Builder:** `SectionBuilder.rich_paragraph(elements)` рядом
      с существующим `.paragraph(text)`.
- [ ] **Экспортёр:** inline-формулы (`<m:oMath>` внутри `<w:r>`),
      `REF`-поля для CrossRef, текстовые run-ы «[N]» / «[N, с. P]» для
      Citation.
- [ ] **Парсер:** распознавание inline-формул и эвристика для
      цитат-ссылок на библиографию.
- [ ] **UI:** inline-редактор параграфа (список run-ов с кнопками
      форматирования + добавление inline-элементов через панель внизу).
- [ ] **Совместимость:** `_normalize_paragraph_state` — старые
      JSON-save из Фазы 2 открываются без миграции.
- [ ] **Undo/Redo:** кольцевой буфер на 50 snapshot'ов с кнопками
      в sidebar.
- [ ] **Auto-save:** дамп state каждые 30 секунд в
      `~/.gostforge/autosave/`, восстановление при запуске UI.
- [ ] **Тесты:** ≥ 40 новых; общая база ≥ 786.
- [ ] **Документация:** `docs/builder.md §4.1`, `docs/architecture.md`,
      README.

**Definition of Done.** Round-trip state → `Document` → `.docx` →
парс → state без потерь inline-элементов, генерируемая
конструктором работа по-прежнему проходит ≥ 29 из 30 проверок профиля
`gost-7.32-2017`, mypy --strict без регрессий.
