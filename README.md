...
- **ЕСКД из конструктора**: `.border(...)` — рамка листа, `.title_block(...)`
  — основная надпись/штамп (ГОСТ 2.104).
- **Авто-наполнение литературы**: `.autofill_references()` —
  упомянутые в тексте ГОСТ/ФЗ добавляются в список источников по
  ГОСТ Р 7.0.100-2018 (с дедупликацией и опц. inline-ссылками [N]).
  Доступно и как `gostforge.autocite.autofill_references(document)`.
...
Семь режимов в одном приложении (переключатель в верху страницы):
**Главная** · **Нормоконтроль** · **Сравнение** · **Конструктор** ·
**Редактор профиля** · **История** · **Документация**.
...
- **ЕСКД в sidebar**: основная надпись (штамп, форма 1/2а) и рамка
  листа задаются для текущей работы.
- **Авто-добавление ГОСТ/ФЗ** в список литературы (чекбокс) с
  уведомлением о добавленных источниках и опц. inline-ссылками [N].
- Экспорт в `.docx`/`.pdf`/`.md`/`.html`; превью PDF в браузере
  (через LibreOffice headless).
- Undo/Redo + версионирование с автосохранением в
  `~/.gostforge/autosave/` (на каждый rerun, троттлинг 30 с).
...
17 endpoints на FastAPI: health, profiles (CRUD), checks, check, fix,
