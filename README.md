...
**116 проверок** в 16 категориях · **35 автофиксеров** · **30 CLI-команд** ·
...
- **Экспортёр `Document → .docx`**. Round-trip без потерь. Настоящие
  ...
  **ЕСКД (ГОСТ 2.104)**: рамка листа (`<w:pgBorders>`) и основная
  надпись (штамп, форма 1/2а) — задаются профилем (своя у каждой
  специальности либо отсутствует).
...
- **116 проверок** в 16 категориях: F (страница), T (текст), S
...
- **35 автофиксеров**: F.01–F.08 (геометрия страницы, формат и позиция
  номера, рамка листа и штамп ЕСКД), T.01–T.14 (шрифт, кегль, интервалы, пробелы, кавычки,
  тире, NBSP), U.01–U.03 (NBSP, знак препинания и точка у единиц
  измерения), H.01–H.04/H.08 (формат заголовков, нумерация,
  точка после/в конце), K.02–K.04/K.06 (нумерация и колонтитулы
  секций: титул без номера, старт основной части, сквозная
  нумерация, отвязка колонтитулов), L.04.
  Применяются по `gostforge fix`, кнопкой «Применить автофиксы» в
  UI или `gostforge apply-fixes`.
...
(страница — включая **рамку листа и основную надпись/штамп ЕСКД**
(ГОСТ 2.104); текст, заголовки, таблицы — включая нумерацию и
...
- **Production-деплой через Docker**: `Dockerfile` (API),
  ...
  образы на `python:3.11-slim`. Кроссплатформенные скрипты
  развёртывания и обновления из git (Linux/macOS/Windows, с
  автоустановкой Docker) — [scripts/README.md](scripts/README.md).
