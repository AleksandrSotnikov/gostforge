...
### Streamlit-WebApp
Четыре режима в одном приложении — открывается на `localhost:8501` через `gostforge ui`.

**Конструктор** теперь содержит:
- **Загрузка готовой работы** (`Загрузить .docx в конструктор` в sidebar) с сразу показанной сводкой нарушений нормоконтроля и кнопкой **«Применить автофиксы»**.
- **Поиск по разделам** с подсветкой совпадений (заголовки, тексты параграфов, подписи таблиц/рисунков, элементы списков, ссылки — на любой глубине).
- **Live-нормоконтроль** в main-области — постоянная сводка нарушений, обновляется при каждом изменении.
- **Прогресс работы**: счётчики разделов / параграфов / таблиц / рисунков / слов / знаков.
- **Шаблоны разделов** в один клик: Введение, Заключение, Реферат, Содержание, Список источников (с `is_bibliography=True`), Приложение А (с `disabled_checks=['*']`), Глава с подразделом.
...
- **Дублирование** и **перемещение разделов** через выпадающий список «Новая позиция» (без drag-and-drop).
...
- **Расширенные настройки стилей** в sidebar (раскрывающаяся секция): поля страницы, основной шрифт+кегль+межстрочный+отступ красной строки+интервалы между абзацами, цвет/UPPERCASE/spacing для заголовков, маркер списков, шаблон нумерации, стиль рамок таблиц.
...
- **Превью PDF** прямо в браузере (через LibreOffice headless) — кнопка «Сгенерировать + превью PDF».
...
- **Undo/Redo** на 50 шагов + автосохранение в `~/.gostforge/autosave/`.
- **Подразделы 3-го уровня** в UI (раскрывающиеся «Пункты» внутри «Подразделов»).

Подробнее — [docs/builder.md](docs/builder.md#4-визуальный-редактор-streamlit).

Остальные режимы:
- **Нормоконтроль** — drag-and-drop, сводка по severity, скачивание аннотированной версии и PDF.
- **История** — список submission-ов из локальной БД с цветным summary, фильтрами и встроенной лентой обсуждения руководитель↔студент.
...
В верхнем переключателе режима — «Нормоконтроль», «Конструктор», «История», «Документация».
