...
**114 проверок в 16 категориях**, **26 автофиксеров**, **1600+ тестов**.
...
- **26 автофиксеров**: безопасные правки (F.01/F.02/F.03 — геометрия страницы, F.04/F.06, T.01–T.14, U.01, H.01/H.02 — формат заголовков, H.03/H.04/H.08, L.04). Применяются по `gostforge fix`, кнопкой «Применить автофиксы» в UI или `gostforge apply-fixes`.
