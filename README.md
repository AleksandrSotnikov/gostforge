...
**114 проверок в 16 категориях**, **23 автофиксера**, **1600+ тестов**.
...
- **23 автофиксера**: безопасные правки (T.01–T.14, U.01, H.01/H.02 — формат заголовков, H.03/H.04/H.08, L.04, F.04/F.06). Применяются по `gostforge fix`, кнопкой «Применить автофиксы» в UI или `gostforge apply-fixes`.
