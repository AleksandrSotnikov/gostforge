...
def render_history_viewer() -> None:
    ...
    with st.expander("ℹ️ Что доступно на этой странице", expanded=False):
        st.markdown(
            """
**Каждый запуск нормоконтроля** (через UI или `gostforge check ...`)
сохраняется как submission. Здесь видно историю.

**Список submission-ов** — expander с цветным summary
`Ne / Nw / Ni` (errors / warnings / info) и временной меткой:

- **Фильтр в sidebar** — по имени файла + «только нерешённые».
- **Раскройте submission** — увидите 2 вкладки:
  - **Нарушения** — список с серьёзностью, кодом, сообщением,
    локацией и подсказкой.
  - **Обсуждение** — лента комментариев руководитель ↔ студент,
    с ролями (студент / руководитель / нормоконтролёр) и
    статусами (открыт / закрыт).

**Комментарии:**

- Кнопка «Закрыть» / «Переоткрыть» — управление статусом.
- Кнопка «Удалить» — удалить запись.
- Форма «Добавить комментарий» — новая реплика в обсуждении.

**Где хранится:** SQLite в `~/.gostforge/gostforge.db`
(или путь из переменной окружения `GOSTFORGE_DB_PATH`).
Чтобы очистить — удалите файл; чтобы перенести — скопируйте.
"""
        )
