...
# Плейсхолдер для персональных/переменных данных в шаблоне титульника:
# ФИО, тема, группа и т. п. студент заполняет вручную. Реальные ФИО в
# шаблон не зашиваем (см. CLAUDE.md: персональные данные не хранятся).
_TITLE_FIELD = "______________________"


def _spo_diploma_title_template() -> dict[str, Any]:
    """Шаблон «Титульный лист + задание» для дипломного проекта СПО.

    Воспроизводит форму титульного листа и задания на дипломный проект
    (БПОУ ОО «Омский промышленно-экономический колледж»). Переменные
    поля — ФИО, тема, группа, специальность, даты — оставлены
    плейсхолдерами ``_TITLE_FIELD`` (заполняются вручную). Раздел
    помечен ``disabled_checks = ["*"]``: он оформляется строго по форме
    учебного заведения и НЕ должен проверяться/исправляться
    нормоконтролем.

    Все строки — обычные параграфы (без принудительного форматирования):
    раздел вставляется «как есть» и не подгоняется движком под ГОСТ.
    """
    ...
    blocks: list[dict[str, Any]] = [
        ...
        p("Министерство образования Омской области"),
        p("бюджетное профессиональное образовательное учреждение Омской области"),
        p("«Омский промышленно-экономический колледж»"),
        p("(БПОУ ОО ОПЭК)"),
        p(),
        p(),
        # --- Гриф «ДОПУЩЕН К ЗАЩИТЕ» ---
        p("ДОПУЩЕН К ЗАЩИТЕ"),
        p("Заместитель директора"),
        p(f"_______________ {_TITLE_FIELD}"),
        p("«___» ______________ 20___ г."),
        p(),
        p(),
        # --- Тип и тема работы ---
        p("ДИПЛОМНЫЙ ПРОЕКТ"),
        p(),
        p(f"Тема: {_TITLE_FIELD}"),
        p(),
        p(f"Специальность: {_TITLE_FIELD}"),
        p(),
        # --- Подписи ---
        p(f"Студент(ка) _______________ {_TITLE_FIELD}"),
        p("(подпись)"),
        p(),
        p(f"Руководитель дипломного проекта _______________ {_TITLE_FIELD}"),
        p("(подпись)        (дата)"),
        p(),
        p(f"Консультант по нормоконтролю _______________ {_TITLE_FIELD}"),
        p("(подпись)        (дата)"),
        p(),
        p(),
        p("Омск, 20___"),
    ...
    return {
        "heading": "Титульный лист (дипломный проект СПО)",
        "blocks": blocks,
        "subsections": [],
...
def _spo_diploma_task_template() -> dict[str, Any]:
    """Шаблон «Задание на дипломный проект» для СПО (вторая страница формы).

    Парный к :func:`_spo_diploma_title_template`. Содержит методические
    пояснения к разделам пояснительной записки (обобщённый текст формы)
    и поля-подписи. Переменные данные — плейсхолдеры. Как и титульник,
    помечен ``disabled_checks = ["*"]`` — оформляется по форме, не по ГОСТ.
    """
    ...
    blocks: list[dict[str, Any]] = [
        ...
        p(),
        p("ЗАДАНИЕ"),
        p("на дипломный проект"),
        p(),
        p(f"по специальности: {_TITLE_FIELD}"),
        p(f"Обучающийся: {_TITLE_FIELD}"),
        p(f"Группа: {_TITLE_FIELD}"),
        p(),
        p(f"Тема дипломного проекта: {_TITLE_FIELD}"),
        p(),
        p("Пояснительная записка"),
        p("МЕТОДИЧЕСКОЕ ОБЕСПЕЧЕНИЕ ВЫПОЛНЕНИЯ ПРОЕКТА"),
        p(
            "Требования к содержанию, объёму, структуре, к оформлению проекта, "
            "а также порядок подготовки и требования к публичной защите в полном "
            "объёме приведены в методических указаниях по выполнению ВКР."
        ),
        p("РАЗРАБОТКА ОСНОВНОЙ ЧАСТИ ПРОЕКТА"),
        p(
            "Проект состоит из пояснительной записки, которая включает в себя "
            "введение, главы, заключение, список использованных источников и "
            "приложения."
        ),
        p("ВВЕДЕНИЕ"),
        p(
            "Во введении содержится обоснование актуальности, практическая "
            "значимость и целесообразность выбранной темы, цель, задачи, предмет "
            "и объект исследования проекта."
        ),
        p("ЗАКЛЮЧЕНИЕ"),
        p(
            "В заключении содержатся основные выводы в соответствии с "
            "поставленными целями и задачами, значимость полученных результатов и "
            "возможность практического применения."
        ),
        p("ПРИЛОЖЕНИЯ"),
        p(
            "Располагаются в конце работы и оформляются в соответствии с "
            "требованиями методических рекомендаций по выполнению ВКР."
        ),
        p(),
        p("Дата выдачи задания «___» ______________ 20___ г."),
        p("Срок сдачи дипломного проекта «___» ______________ 20___ г."),
        p(f"Руководитель дипломного проекта _______________ / {_TITLE_FIELD} /"),
        p("Одобрено предметно-цикловой комиссией: протокол № ___ от __________"),
        p(f"Председатель ПЦК _______________ / {_TITLE_FIELD} /"),
        p(f"Задание получил обучающийся _______________ / {_TITLE_FIELD} /"),
        p("«___» ______________ 20___ г."),
    ]

    return {
        "heading": "Задание на дипломный проект (СПО)",
        "blocks": blocks,
        "subsections": [],
        "disabled_checks": ["*"],
...
_SECTION_TEMPLATES: dict[str, tuple[str, Any]] = {
    ...
    "title_spo": (
        "Титульный лист (дипломный проект СПО)",
        _spo_diploma_title_template,
    ),
    "task_spo": (
        "Задание на дипломный проект (СПО)",
        _spo_diploma_task_template,
    ),
    "intro": (
        "Введение",
        lambda: {
...
_FRONT_TEMPLATES = frozenset({"title", "title_spo", "task_spo"})
